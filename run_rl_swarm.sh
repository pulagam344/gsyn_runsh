...
if [ "$CONNECT_TO_TESTNET" = true ]; then
    ...
    if open http://localhost:PORT_CHANGE 2> /dev/null; then
        echo_green ">> Successfully opened http://localhost:PORT_CHANGE in your default browser."
    else
        echo ">> Failed to open http://localhost:PORT_CHANGE. Please open it manually."
    fi
